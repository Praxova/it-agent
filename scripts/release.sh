...
Options:
  ...
  --skip-llm         Skip building/deploying LLM server image
  --dry-run          Show what would happen without executing
  -h, --help         Show this help message

Examples:
  $(basename "$0") --host deploy@proxmox-docker
  $(basename "$0") --host deploy@10.0.0.50 --tag v1.2.3 --skip-llm
...
BUILD_ARGS=()
[[ "$SKIP_LLM" == "true" ]] && BUILD_ARGS+=(--skip-llm)
BUILD_ARGS+=("$TAG")
...
# ── Deploy ───────────────────────────────────────────────────────
DEPLOY_ARGS=()
[[ "$SKIP_LLM" == "true" ]] && DEPLOY_ARGS+=(--skip-llm)
